        ...
        <template v-if="comments.length != 0" v-for="(comment, index) in comments">
          ...
          <v-card-actions>
            <v-btn
              ...
              @click.prevent="updateLike(comment.cmtId, index)"
            >
...
export default {
  ...
  methods: {
    ...
    isExistDocEntry: function(docEntry) {
      ...
      if (docEntry === null || docEntry === undefined) return false;
      if (docEntry.length > 0) return true;
      return false;
    },
    onCmReady(cm) {
