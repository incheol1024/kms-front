...
export default {
  ...
  methods: {
    ...
    getPage_board: function(page) {
      ...
      if (this.curSite.siteId === 0) return;
      return axios.get(
        `site/${_this.id}/${_this.curSite.siteId}/${
          _this.curProject.projectId
        }`,
        {
          params: page
        }
    ...
    addBoard: function() {
      let _this = this;
      router.push(
        `/sites/write/${_this.id}/${_this.curSite.siteId}/${
          _this.curProject.projectId
        }/0`
      );
    },
    ...
    deleteBoard: function(item) {
      ...
      if (confirm("삭제하시겠습니까?"))
        return axios.delete(
          `site/${_this.curSite.siteId}/${_this.curProject.projectId}/${
            item.boardId
          }`
        );
    }
